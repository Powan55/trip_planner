...
// which are per-route and all 20 differ.
...
async function buildManifest() {
  const manifest = {
    ...
    // route resolves (trailingSlash:true). `share/index.html` is picked up by the out/ walk
    // like every other route, so the receiving surface is precached and resolves offline.
    share_target: {
 ...
 * hashed (${PRECACHE}) over every precached file's path and bytes; any byte
 * change yields a new cache name and drives the update-available flow in the
 * registrar.
 */
...
// route's URL is rewritten onto it here rather than storing 19 more copies of it.
