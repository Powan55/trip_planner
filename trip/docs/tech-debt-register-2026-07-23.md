...
Basis: a full read of nextjs_space/{lib,core,components,hooks,app,scripts,e2e} plus worker/src, and these real runs:

- npx tsc --noEmit → exit 0, zero errors
- npx vitest run → 119 files / 1336 tests passed (19.5s)
- npm run lint → does not run (interactive prompt; no ESLint config exists, see TD-02)
...
Severity counts: CRITICAL 0 · MAJOR 4 · MINOR 6 · NICE-TO-HAVE 2 · SKIP 10
...
- Firestore doc reads sanitized: `lib/trips-remote.ts:77-80`, `lib/itinerary-remote.ts:123-129`.
- Every localStorage `JSON.parse` is try/caught with a fallback: `core/storage/gateway.ts:434-443`, `core/trips/registry.ts:112-128`, `lib/currency-rate.ts:45-55`, `core/vault/load-save.ts:128`, `lib/expense-export.ts:57`.
- Worker validates origin/method/token-shape/body-size/JSON (`worker/src/index.ts:44-96`), and `fetchChatCompletion` is total: every provider fetch is caught (`worker/src/providers.ts:61-122`).
...
- **Status: RESOLVED in S258.** `{isDefault && <ConciergeChat />}` in navbar (mount-safe pattern); the gate is exercised on a concierge-enabled test build, and is env-dormant in deploys.
...
### TD-13: (filed at S266, 2026-07-23) expense-dialog leg toggle hardcodes nepal/japan
- Surfaced during S266: `components/expense-dialog.tsx` hardcodes the leg toggle to `['nepal','japan']`. A custom trip has one synthesized leg, so the toggle is N×J-specific. Latent leak, cosmetic on custom trips, since the roster itself is fixed by TD-03/D-223. Not in the 2026-07-23 run; a future item if custom-trip budgeting is pursued.
...
4. calendar-planner.tsx at 1806 lines (settings-panel.tsx 1027). Big but cohesive, tsc-clean, covered by the heaviest e2e suites in the repo. Splitting is churn risk with no bug behind it. Revisit only when a slice forces entry.
5. A shared stripUndefined helper for the 5 duplicated one-line JSON round-trip sanitizers (`budget-remote.ts:41`, `docs-remote.ts:46`, `expenses-remote.ts:48`, `itinerary-remote.ts:165`, `trips-remote.ts:56,114`). Each is one commented line, and a shared helper couples five locked-pattern modules to save ~5 lines.
