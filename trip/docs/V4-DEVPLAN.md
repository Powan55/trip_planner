> **Historical: the M17/v4 plan, completed and superseded. v5 (`V5-DEVPLAN.md`) rebuilt this scope, and the final version's scope lives in `V-FINAL-DEVPLAN.md`. Its dependency pins are dead: line 487 reads next 14.2.35 / react 18.2.0 / framer-motion 10 / maplibre 4, against the shipped next 15.5.20 / react 19.2.7 / framer-motion 12.42.2 / maplibre-gl 5.24.0. Section 7's gate G5 (the `firestore.rules` uid-allowlist, FU-18) was retired outright by D-205. Kept because DECISIONS.md, `photo-storage-blueprint.md`, `sync-everywhere-blueprint.md` and `V-FINAL-DEVPLAN.md` all cite it by name.**
>
> **Deletion was ordered and is deliberately not done. Do not "finish the job" without reading this.** S418's ledger-disposition clause says to delete this file. That clause was written without checking who points at it. Deleting it was tried and measured: `node scripts/marker-check.mjs` goes from clean to **4 dangling-doc-ref failures**. They are `DECISIONS.md:1035` (the section 7 gates, resolved under the D-129 autonomy grant), `DECISIONS.md` D-167's proof-of-absence, `V-FINAL-DEVPLAN.md:186` (S418's own row, which names this file in the very instruction to delete it), and `photo-storage-blueprint.md:3` (which states this file is what it makes buildable). `sync-everywhere-blueprint.md:320` cites it too, extensionless, so it survives the gate and becomes a dead pointer for a human instead. Every one of those is a real provenance line in a document whose job is provenance; stripping them to satisfy the delete order would trade a stale-but-true file for four falsified citations. The banner is the disposition.
...
- **Goal:** search-within-map (client-side over known places, behind a search icon); a live location toggle (browser geolocation, default off, permission-prompted, nothing persisted — still true of THIS toggle after D-320; see the amendment note in section 4); directions links in popups (external maps URL, D-074 href discipline); and the honest "schematic line, not a route" caveat on the day overlay.
