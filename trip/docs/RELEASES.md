...
The redesign lands, and it is the largest single release in this repo's history: 55 commits closing 26 issues. The app that ships here does not look like the one before it.
...
**Smaller, and still worth naming.** Inner pages get photo headers and their own accent (#3). Home gets one scrim over the hero and a live stat row under it (#26), and the weather panel becomes a curated inspiration gallery (#21). Saved places follow you between phones (#17). There is a night-before readiness check that never guesses (#20) — it precaches the map *engine*, not basemap tiles, and says so. The phrasebook carries native script on every row (#2). Day one is named New York (#6). The shared-trip door sends a visitor to signup instead of asking for a key they cannot have (#70).
