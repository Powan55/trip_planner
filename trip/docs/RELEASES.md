...
- Docs: `worker/README.md` no longer contains an instruction that would re-arm the disabled search leg. A `secret put` step sat five lines below the step requiring that secret's deletion.
...
- The "Last Train" visual overhaul: new dark ramp, Geist plus a conditional Instrument Serif display face (gated on a font-leak audit that found and repaired four leak sites), retuned radii, elevation and motion durations, then a three-bucket palette reconciliation (cyan for interaction, gold for status, ink for decoration) swept twice, the second time against the merged tree.
