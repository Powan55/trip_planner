...
19 retimed or retitled across the Osaka and Tokyo days. Only fresh devices pick up the seed; saved
plans are untouched.

Known ceiling: the plan digests to 10113 chars against the 9500 concierge cap, so the last days are
cut from what the AI sees until the cap and the Worker's truncate length are raised together.
