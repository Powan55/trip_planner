...
## 5. Quick-add: `components/quick-add-fab.tsx` (most routes, `<md` only) + `components/quick-add-button.tsx` (routes: `/guides`, `/checklist`, all widths)

| testid | element | notes |
|---|---|---|
| `quick-add-fab` | the floating action `<button>` | `md:hidden`. Hides itself while `document.body.dataset.dialogOpen === '1'` (i.e. while any dialog/sheet is open); that is existing behavior, unchanged. It is route-suppressed (absent from the DOM, not merely hidden) under `/travel` (D-164), under `/plan` since S357C (which has its own S357A sticky composer), and under every entry in `NON_ADD_ROUTES` — `/trips`, `/packing`, `/safety`, `/more`, `/recap`, `/profile`, `/flights`, and `/guides` + `/checklist` since D-534. So a spec that needs the FAB must host itself on a route where it renders (`/` is the canonical choice); `/plan/`, `/guides/` and `/checklist/` are no longer such routes. |
| `quick-add-button` | the in-page "Add to plan" `<button>` | Where the add intent went on the two routes D-534 suppressed the FAB on: rendered at the foot of `/guides` and `/checklist`. NOT `md:hidden` — it is page content, so it is present at every width, including desktop, where those routes previously had no add affordance at all. Dispatches the same `quickadd:open` CustomEvent with the same preset date as the FAB, so it opens the same `add-item-dialog` (section 6) via `quick-add-host`. On `/guides` it renders INSIDE `DefaultTripOnly`, so on a custom trip it is absent and `default-trip-only-empty-state` is there instead; on `/checklist` it is ungated. |
