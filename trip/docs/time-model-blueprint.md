...
> (D-137 amendment, S393), the deletion of `isPastAtPlace`, and the move from wall-clock
> minutes to absolute instants in sort and clash. Current behaviour lives in the code and
> in `DECISIONS.md`.
...
// DELETED: its one-line body — `placeWallClockToUtcMs(...) < nowUtcMs` — is inlined
...
**Built, then amended in code:** `isPastAtPlace` shipped as designed and was later DELETED — its one-line body (`placeWallClockToUtcMs(...) < nowUtcMs`) is inlined at its single call site in `lib/whats-next.ts`, which now uses that instant as BOTH the past-gate and the ranking key (the two disagreed once a day held items in different zones). The decision's substance is unchanged: one offset-injected instant compare, at-now still counts as upcoming. Only the named helper is gone; `DECISIONS.md` D-140 still names it and has not been amended.
