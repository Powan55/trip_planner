 ...
 *
 * Budget originally had no hook — `components/budget-panel.tsx` read/wrote the `BudgetModel` ad hoc.
 ...
 * every domain has (dual-layer reactivity, fresh-base commit).
 *
 * ── Sync (LWW-per-field singleton) ───────────────────────────────────────────────
 * The factory carries the budget `SyncPort` (`budgetSyncPort`): the commit tail fires
 ...
 * `sync.fieldHlc` is ever written, so the key-10 bytes are byte-for-byte unchanged from before sync
 * existed (the panel's UX, mutators, and money math are all unchanged — they still call
 * `commit(() => next)`).
 */
...
export interface BudgetStore {
  ...
  /**
   * Reset the whole budget to the seeded default (settings page). Dormant: a plain local seed write
   * (commit passthrough, byte-identical). With sync on: commit stamps the changed leaf paths with a fresh
   * HLC (`stampBudgetChanges`), so the reset-to-seed wins the next per-field LWW merge — a set field
   * cleared back to its seed propagates and sticks, not a blind wipe the next snapshot would overwrite.
   */
  reset(): void;
}
...
const useBudgetStore = createReactiveStore<BudgetModel>({
  ...
  storageKeys: [STORAGE_KEYS.budget],
...
export function useBudget(): BudgetStore {
  ...
  // Stamp the changed leaf paths under sync; passthrough when dormant (byte-identity preserved). Stamping
  ...
  const commit = useCallback((compute: (current: BudgetModel) => BudgetModel) => {
    rawCommit((current) => {
      ...
      if (!syncEnabled()) return next; // dormant: no sync.fieldHlc, byte-identical to pre-sync behavior
