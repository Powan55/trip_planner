...
export interface ReactiveStoreConfig<T> {
   ...
   * (STORAGE_KEYS.*), never a literal. May be a FUNCTION evaluated at event
   * time when the key is pack-dependent: the itinerary domain passes
   * `() => [keyFor('itinerary')]` so a non-default trip pack — whose on-disk key is
   * `trip:{id}:itinerary`, not the grandfathered default literal — still gets cross-tab
   * reactivity. Default pack: `keyFor` returns the legacy literal, byte-identical. */
  storageKeys: readonly string[] | (() => readonly string[]);
  /** The existing per-domain StoragePort. (key-presence, []-survives, quarantine,
   * sanitize-on-load) live INSIDE the impl — the factory is agnostic. */
