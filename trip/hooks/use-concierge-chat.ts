  ...
  const send = useCallback(
    async (message: string) => {
      ...
      try {
        ...
        // CORS rejection) rejects with `TypeError: Failed to fetch`, and that exact machine string
        // was being rendered in the panel's error row. The offline pre-check above only catches
        // the case where the device knows it is offline; online-but-dead lands here.
