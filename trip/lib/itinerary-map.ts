...
// 164 seed items + 142 registry cards + one row per marker, 462 haystacks: NOT ONE resolved pin
...
export const NAME_INDEX = MAP_MARKERS.map((mk) => {
  ...
  // place, not its container. RE-MEASURED on the 164-item seed against the 156 markers and the
  // matcher below: `osaka` is the only alias the guard still drops, and re-admitting it puts 21
  // Japan-leg items on jp-osaka-castle while losing 0 correct ones. It was 19 at 27 markers —
  // the guard carries more weight now, not less.
...
export function matchMarker(item: ItineraryItem): MapMarker | null {
  ...
  //
  // TITLE FIRST, then title+location. Alias length is only a proxy for specificity, and it
  // breaks down across the two fields: "Breakfast at OR2K Restaurant" / `location: 'Thamel'`
  // names its venue in the title, but `thamel` (6) outranks `or2k` (4) and the dinner plotted
  // at the district. What the item calls ITSELF beats where it says it is, whatever the two
  // names happen to measure. Within each pass the longest alias still wins, so the district
  // shadowing this replaced (a venue named only in `location`) stays fixed by pass 2.
  const title = item.title.toLowerCase();
  for (const { marker, re } of ALIAS_INDEX) {
    if (re.test(title)) return marker;
  }
  const hay = `${title} ${(item.location ?? '').toLowerCase()}`;
  for (const { marker, re } of ALIAS_INDEX) {
