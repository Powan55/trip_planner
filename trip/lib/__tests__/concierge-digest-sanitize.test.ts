...
describe('buildTripDigest — the strip is enforced by construction', () => {
  ...
  // from the trip's meta doc. This build's `buildTripDigest` has no per-day-dropping truncation
  // (it hard-caps the whole string at `DIGEST_CAP` instead), so there is no `omitted` field here.
