 ...
 *  • np-thamel's `thamel` alias shadowed Thamel VENUES, because `matchMarker` returned the
 *    first hit in MAP_MARKERS order and np-thamel sits at index 4. → this rule goes red on
 *    every one of them.
 *  • It went red again on the canonical seed after the district shadowing was fixed: "Breakfast
 *    at OR2K Restaurant" / `location: 'Thamel'` still lost, because longest-alias-wins ranks
 *    `thamel` (6) over `or2k` (4) and both fields were one haystack. That is what forced the
 *    title-first pass in `matchMarker`.
 *  • A marker carrying a qualifier no itinerary text contains ("Phulchowki Summit") is INVISIBLE
 *    to this rule — the item is not covered rather than failing. The double-space test at the
 *    bottom catches one variant of it and nothing catches the rest. Closing it properly needs a
 *    declared card-id → marker-id join, which this repo does not have: seed items carry a
 *    REGISTRY card id in `sourceId` ('na18'), never a marker id.
 */
describe('the marker vocabulary is self-consistent with the seed', () => {
   ...
   *
   * Exclusions: NONE. Measured on the current seed — 37 of 164 items carry a marker name in
  ...
  it("every seed item that names a marker in its title resolves to that marker's own pin", () => {
    ...
    // do not lower it — find out why items stopped naming their markers. 37 measured against the
    // canonical seed; the floor sits under that, not on it, so ordinary content edits don't trip it.
    expect(covered, 'the derived rule covers almost nothing — it has gone vacuous').toBeGreaterThanOrEqual(30);
  });
