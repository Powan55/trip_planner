 ...
 * `navbar.tsx` and the mobile `bottom-tab-bar.tsx` (the two components previously
 * each carried a byte-identical local copy of this array + the route-match helpers
 * below).
 *
 * The nav is ROUTE-driven, trailing-slash canonical hrefs (`trailingSlash:true`).
 * next/link handles basePath; active state comes from usePathname() (which EXCLUDES
 * basePath), so the whole nav is basePath-agnostic.
 *
 * Flights was added as a sixth route-driven item (moved off Home onto its own
 ...
 *
 * `/journal`, `/safety`, `/recap` were shipped with their nav wiring deliberately
 * deferred. Adding them to NAV_ITEMS naively would push the mobile tab bar to 9 tabs —
 * at a 360px viewport that's ≈40px each, BELOW the ≥44px floor. So NAV_ITEMS stays the
 * full 9-item catalog (consumed by the constrained-width-agnostic surfaces: the mobile
 * hamburger panel + the command palette), while the 6 "primary" (daily-use) items are
 * re-exported as `PRIMARY_NAV_ITEMS` for the two width/slot-constrained surfaces (the
 * bottom tab bar and the desktop top row).
 */
export type NavItem = {
  label: string;
  href: string;
