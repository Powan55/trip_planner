...
/**
 * Author filter control — a presentational, READ-ONLY view filter that narrows the
 * calendar and timeline item lists to All / "My edits" / a specific traveler, using
 * the existing `createdBy` / `updatedBy` attribution. It NEVER mutates stored data:
 * selecting an option only updates the shared in-memory selection (lib/author-filter),
 * which both surfaces read.
 *
 ...
 *
 * A11y: a labeled segmented control — a `<div role="group">` with an accessible name,
 * options as `<button aria-pressed>` with visible `focus-visible` rings, fully
 * keyboard-operable (native button semantics: Tab to reach, Enter/Space to pick).
