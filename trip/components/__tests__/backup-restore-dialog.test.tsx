...
describe('BackupRestore — the outcome reports what was actually restored', () => {
  ...
  it('a restore that found nothing is a FAILURE (`ok:false`, refused before any write) — no success copy, and no reload', async () => {
    // `importTripBackup` refuses this shape itself now, before Phase B writes anything (a
    // `photos.meta: []`-only envelope used to still run `savePhotos([])` and wipe the live photo
    // index while reporting `restored: []`). The panel's job is just to render the refusal.
