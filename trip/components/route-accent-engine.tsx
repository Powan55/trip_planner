...
/**
 * Route-driven warm/cool accent engine ( — formerly
 * `scroll-accent-engine.tsx`,).
 *
 ...
 *
 * is RETIRED here: the old outer rAF poll for
 ...
 * ( general rule — a layout-root island consuming a DOM-anchored hook
 * must defer binding — remains true as a pattern; it just has no live consumer.)
 *
 * Motion: a route-linked colour shift is NOT
...
// not reproduce the brand values (gold rounds to 235,193,76 but pins
// 240,199,96 — a visible 20-pt blue gap), so every SETTLED accent must write the
// authored literal verbatim to equal byte-for-byte. (Derivation is
...
export default function RouteAccentEngine() {
  ...
  // ambient trip phase: stamp `data-trip-phase` on <html> so globals.css's
  // ambient palette overrides (html[data-trip-phase='nepal'|'japan']) cascade into
  // the --gradient-aurora lead stops. Read-only consumption of the clock
  // (getTodayInTrip resolves the `?today=` override), so the visual baselines'
  // frozen `?today=2026-11-15` deterministically yields 'pre'. Set once per load —
  // the phase is a calendar-day fact, and every load re-derives it.
  useEffect(() => {
  ...
  // on an authored literal). Seeded to gold.
