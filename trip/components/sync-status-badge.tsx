 ...
 * dormant/guest build, with no separate gate check duplicated here.
 *
 * THREE STATES, not two (#267). "pending" tells a traveler their edits will land on their own;
 * for a change the security rules REFUSED that is false and no amount of waiting fixes it, so a
 * refusal gets its own wording and its own `data-state`. `blocked` is a SUBSET of `pending` (a
 * refused chunk is never acked), so it is checked first and the `show` condition needs no clause
 * for it.
 *
...
export function SyncStatusBadge() {
  const { pending, blocked, lastAckAt } = useSyncStatus();
  ...
  // #267: a REFUSED change reads as pending forever, which is the one thing this pill must never
  // say. `blocked` is a subset of `pending` (a refused chunk is never acked), so `show` above is
  // already true whenever it is non-zero and needs no extra clause — but the wording has to change,
  // because "pending" promises this clears itself and a rules refusal never will.
