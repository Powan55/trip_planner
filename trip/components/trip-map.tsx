...
// (/map, and /plan in), the CSS ships only on those routes.
...
function MarkerPopupContent({ marker, enableFavorite }: { marker: MapMarker; enableFavorite?: boolean }) {
 ...
 {/*: `/40`→`/55` (mirrors the loading-label fix above) — axe
...
const TripMap = forwardRef<TripMapHandle, TripMapProps>(function TripMap(
 ...
 ? createPortal(
 <MarkerPopupContent marker={popupMarker} enableFavorite={enablePopupFavorite} />,
 popupNode,
 )
 : null}

 {/* Scoped dark-brand overrides for the MapLibre popup + controls. A plain
 <style> element (local to this component, not globals.css which the global stylesheet
