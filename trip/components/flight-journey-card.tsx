...
// NEVER from booking labels —). Kept self-contained so a later slice can embed it
...
const PHASE: Record<FlightPhase, { label: string; strip: string; Icon: typeof Timer }> = {
  upcoming: { label: 'Upcoming', strip: 'bg-cyan-500/12 text-cyan-200 border-cyan-500/25', Icon: Timer },
...
const VERDICT: Record<'relaxed' | 'normal' | 'tight', { label: string; cls: string }> = {
  ...
  normal: { label: 'Normal', cls: 'bg-cyan-500/15 text-cyan-200 border border-cyan-500/25' },
...
const RAIL_LINK =
  'inline-flex min-h-[36px] items-center gap-1.5 rounded-lg bg-white/5 px-3 text-[11px] font-medium text-white/70 outline-none transition-colors hover:bg-white/10 hover:text-white focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:outline-none';
