    ...
    >
      <div className="flex shrink-0 items-center justify-between gap-3 px-4 pb-2 pt-4">
        ...
        <button
          ...
          className="ml-auto inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg text-ink-mid outline-none transition-colors duration-200 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto px-4 pb-4">
        {photo && !missing && url ? (
          ...
          >
            ...
            <span className="text-sm text-ink-mid">{photo.caption ?? photo.altText}</span>
            <span className="sr-only">Photo no longer on this device</span>
          </div>
        ) : (
          <div className="h-64 w-64 rounded-lg bg-white/[0.04] motion-safe:animate-pulse" aria-hidden="true" />
        )}
      </div>

      {photo?.caption && !missing && (
        <p className="shrink-0 px-4 pb-4 text-center text-sm text-ink-mid">{photo.caption}</p>
      )}
    </Sheet>
  );
}
